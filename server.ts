...
app.post("/api/proxy/openrouter", async (req, res, next) => {
  try {
    ...
    while (attempts < maxAttempts && !success) {
      ...
      attempts++;
      try {
        console.log(`[OpenRouter Proxy Route] Attempt ${attempts}: Using key index ${state.index} (${state.maskedKey}) with model ${targetModel}`);
...
Định dạng JSON:
{
  ...
  app.post("/api/agent3/chat", aiCooldownMiddleware, async (req, res, next) => {
    try {
      const { message, history, context, mode, mcqData, difficulty, sessionId, responseMode, responseStyle, isConciseMode } = req.body;
...
- Chỉ trả lời thẳng vào vấn đề, bỏ qua hoàn toàn các lời dẫn dắt, giải thích vòng vo hay hỏi ngược lại dài dòng. Trả lời cực kỳ ngắn gọn (chỉ 1-2 câu).`;
      ...
      if (responseMode === "direct") {
        systemPrompt = `Bạn là Agent 3 - 'Socrates AI Coach', đang ở chế độ TRẢ LỜI TRỰC DIỆN (DIRECT MODE). QUY TẮC BẮT BUỘC CỐT LÕI:
1. TRẢ LỜI TRỰC TIẾP. Cung cấp câu trả lời, công thức, khái niệm hoặc đáp án cụ thể ngay lập tức để học sinh củng cố kiến thức. KHÔNG hỏi ngược lại học sinh. Không dùng phương pháp Socratic để khơi gợi nữa.
