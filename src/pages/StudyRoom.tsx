...
export default function StudyRoom() {
  ...
  useEffect(() => {
    if (rawDeck && user) {
      ...
      const isSystem = systemDecks.includes(rawDeck.id);
      const isCreatedBySelf = rawDeck.createdBy === user.id;
      const isCreatedByTeacher = rawDeck.creatorRole === "teacher" || rawDeck.creatorRole === "Admin" || rawDeck.creatorRole === "admin";
  ...
  const handleSaveEdit = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        updatedDocCards = [
          { ...currentCard, front: editFront, back: editBack },
        ]; // dummy fallback
      }
            ...
            <div className="flex-1 overflow-y-auto mt-4 pr-1 space-y-3 scroll-smooth">
              {filteredCards.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  <p className="text-sm opacity-60">Không tìm thấy thẻ học nào khớp với từ khóa của mày.</p>
                  <button
                    onClick={() => setListSearchQuery("")}
