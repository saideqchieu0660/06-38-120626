...
import { Copy, ExternalLink, Database, Check, Sparkles, X, Edit3, Trash2, Plus, RefreshCw, Layers, Terminal, AlertTriangle, AlertCircle, FileUp, FileText, Loader2, Activity } from "lucide-react";
...
export default function ManualFlashcardImporter() {
  ...
  const logThrottleTimeoutRef = useRef<any>(null);
  const isCanceledRef = useRef<boolean>(false);
  const activeControllersRef = useRef<Set<AbortController>>(new Set());

  // Step cancellation & skip handlers for large vocabulary processing
  const [activeProcessingChunkIndex, setActiveProcessingChunkIndex] = useState<number | null>(null);
  const currentActiveChunkControllersRef = useRef<Map<number, AbortController>>(new Map());

  const handleSkipActiveChunk = (chunkIndex: number) => {
    const controller = currentActiveChunkControllersRef.current.get(chunkIndex);
    if (controller) {
      allLogsRef.current.push(`⚠️ [Người dùng] Click bỏ qua phân đoạn #${chunkIndex + 1} đang kết xuất...`);
      controller.abort();
    }
  };
  ...
  const handleCancelAutomatedPipeline = () => {
    ...
    allLogsRef.current.push(`🛑 Tiến trình tự động hóa đã bị hủy bỏ khẩn cấp bởi người dùng thành công! Mở khóa các tùy chọn.`);
  ...
  React.useEffect(() => {
    return () => {
      if (logThrottleTimeoutRef.current) {
        clearTimeout(logThrottleTimeoutRef.current);
      }
      for (const controller of activeControllersRef.current) {
        if (controller) {
          controller.abort();
  ...
  React.useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        if (parsed.allGeneratedCards && Array.isArray(parsed.allGeneratedCards)) {
  ...
  const handleRetrySingleFailedChunk = async (errIdx: number, originalIndex: number, currentText: string) => {
    ...
    try {
      ...
      if (!res.ok || !data.success || data.rawText === undefined) {
        throw new Error(data.message || "Tạo thất bại. Vui lòng kiểm tra lại Keys hoặc nội dung.");
      }

      const rawLinesStr = data.rawText as string;
      const resLines = rawLinesStr.split("\n").map(l => l.trim()).filter(l => l.length > 0);
      ...
      if (cardsInChunk.length < exactCount) {
        throw new Error(`Data Loss: Yêu cầu ${exactCount} thẻ nhưng chỉ nhận được ${cardsInChunk.length} thẻ hợp lệ.`);
      }
      ...
      const targetDeckForBatch = isAddToExisting && selectedExistingDeckId ? store.getDeck(selectedExistingDeckId) : null;
      if (targetDeckForBatch) {
        currentBatchDeckId = targetDeckForBatch.id;
      }

      const currentBatchSubject = targetDeckForBatch ? (targetDeckForBatch.subject || "Tự chọn") : (deckSubject || "Tự chọn");
      ...
      if (deckInStore) {
        ...
        const newDeckObj = {
          ...
          title: deckTitle.trim() || savedPipeline?.deckTitle || "Bộ thẻ Tự động hóa",
          subject: currentBatchSubject,
      ...
      allLogsRef.current.push(`💖 Phục hồi lỗi phân đoạn #${originalIndex} thành công! Nạp thành công ${mappedBatch.length} thẻ.`);
      setAutoProgress((prev) => prev ? {
        ...prev,
  ...
  const handleRetryAllFailedChunks = async () => {
    ...
    allLogsRef.current.push(`⚡ Khởi chạy tiến trình băm lại hàng loạt ${errorsToRetry.length} phân đoạn lỗi đang cách ly...`);

    for (let i = 0; i < errorsToRetry.length; i++) {
      const errItem = errorsToRetry[i];
      try {
        const lines = errItem.content.split("\n").filter(l => l.trim().length > 0);
        const exactCount = lines.length;
        ...
        if (cardsInChunk.length < exactCount) {
          throw new Error(`Data Loss: Yêu cầu ${exactCount} thẻ nhưng chỉ nhận được ${cardsInChunk.length}`);
        }
        ...
        successCountLocal++;
        allLogsRef.current.push(`💚 [Hồi phục hàng loạt] Tạo thành công phân đoạn lỗi #${errItem.chunkIndex}.`);
      } catch (err: any) {
        console.error(err);
        remainingErrors.push(errItem);
        allLogsRef.current.push(`⚠️ [Hồi phục hàng loạt] Phân đoạn lỗi #${errItem.chunkIndex} bọc lỗi tiếp: ${err.message || err}`);
      }
    }
    ...
    alert(`Hoàn thành băm lại hàng loạt phân đoạn lỗi! Phục hồi thành công: ${successCountLocal}/${errorsToRetry.length} phân đoạn.`);
  };
  ...
  const handleCleanAndDeduplicate = async (inputText: string) => {
    ...
    for (const rawLine of lines) {
      ...
      if (existingFronts.has(lowerLine) || (coreWordMatch && existingFronts.has(coreWordMatch))) {
...
Acknowledge this protocol. Execute all text transformations deterministically at 100% precision. Analyze the uploaded files or inputs and start processing the data matrix now.`;
    ...
    try {
      ...
      if (parsedData && typeof parsedData === "object" && !Array.isArray(parsedData)) {
        ...
        if (Array.isArray(parsedData.cards)) {
          ...
          throw new Error("Không tìm thấy danh sách thẻ học (mảng 'cards') trong cấu trúc dữ liệu!");
        }
      }
      ...
      console.error(err);
      setError(`Lỗi Parse JSON: ${err.message || "Định dạng JSON không hợp lệ"}. Vui lòng kiểm tra lại cú pháp.`);
    }
  };
  ...
  const handleUpdateAutoCard = async (id: string, field: 'front' | 'back', value: string) => {
    const updated = autoCreatedCards.map(c => c.id === id ? { ...c, [field]: value } : c);
  ...
  const handleJsonFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      if (selected.size > 2 * 1024 * 1024) {
        setError("Kích thước tệp tin JSON/TXT quá lớn (Vui lòng chọn tệp < 2MB).");
        return;
      }
  ...
  const handleParseJsonWithAi = async (textToParse?: string | React.MouseEvent) => {
    const text = (textToParse && typeof textToParse === "string") ? textToParse : jsonInput;
    if (!text.trim()) return;
    setError(null);
    setSuccessCount(null);
    ...
    try {
      ...
      console.error(err);
      setError(`Lỗi chuẩn hóa AI: ${err.message || "Không thể khởi động kênh chuẩn hóa."}. Vui lòng thử lại hoặc dùng "Parse JSON thủ công" bên dưới.`);
    } finally {
  ...
  const handleAutomatedPipeline = async (resumeState?: typeof savedPipeline) => {
    ...
    if (isResuming && resumeState) {
      ...
      const { cleanedLines, stats } = await handleCleanAndDeduplicate(rawTextLines);
      if (cleanedLines.length === 0) {
        setError(`Lọc hoàn tất: Tất cả ${stats.initial} dòng nhập vào đều trùng lặp hoặc chứa từ vựng đã có trong hệ thống.`);
        setIsAutomating(false);
        return;
      }
    ...
    try {
      ...
      let chunkSize = 20;
      let concurrency = 1; // Strict serial conveyor queue to insulate keys from 429 & trigger instant responses

      const chunkArray: string[][] = [];
      for (let i = 0; i < finalRawLines.length; i += chunkSize) {
        chunkArray.push(finalRawLines.slice(i, i + chunkSize));
      }
      ...
      const pushLog = (msg: string, immediate = false) => {
        ...
        if (immediate) {
          if (logThrottleTimeoutRef.current) {
            clearTimeout(logThrottleTimeoutRef.current);
            logThrottleTimeoutRef.current = null;
          }
          ...
          if (!logThrottleTimeoutRef.current) {
            logThrottleTimeoutRef.current = setTimeout(() => {
              setAutoProgress((prev) => {
                if (!prev) return null;
                return {
                  ...prev,
              ...
              logThrottleTimeoutRef.current = null;
            }, 500); // UI log state throttled to 500ms
          }
        }
      };

      pushLog(`🔮 [SEMAPHORE STARTING] Kích hoạt luồng trích xuất tuần tự (Chunk Size: ${chunkSize} dòng) bảo đảm an toàn, bứt tốc 1000 thẻ.`);
      ...
      for (let cIdx = startChunk; cIdx < totalChunks; cIdx++) {
        ...
        // 1. Rest delay with stagger logic
          ...
          if (cIdx % 3 === 0) {
            try {
              ...
              if (currentKeys && !isCanceledRef.current) {
                const activeKeys = currentKeys.filter((k: any) => k.status === "active");
                const coolingKeys = currentKeys.filter((k: any) => k.status === "rate_limited");
                
                if (coolingKeys.length > 0) {
                  pushLog(`❄️ Hiện có ${coolingKeys.length} Keys đang giãn cách làm dịu...`, true);
                }

                if (activeKeys.length === 0) {
                  pushLog(`🚨 CẢNH BÁO: Tất cả Keys bận rộn! Chờ 15s để làm dịu...`, true);
                  let coolRemain = 15;
          ...
          pushLog(`⚡ [Phân đoạn ${cIdx + 1}/${totalChunks}] Đang băm từ vựng (${exactCount} thẻ)...`);
          setAutoProgress((prev) => prev ? { ...prev, currentChunk: completedCount } : null);
          ...
          while (!success && attempt < maxRetries && !isCanceledRef.current) {
            ...
            try {
              ...
              if (data.success && data.rawText !== undefined) {
                const rawLinesStr = data.rawText as string;
                const lines = rawLinesStr.split("\n").map(l => l.trim()).filter(l => l.length > 0);
                
                cardsInChunk = [];
                for (const line of lines) {
                  // e.g. front ||| ipa ||| wordForm ||| back ||| example ||| origin
                  const parts = line.split("|||").map(p => p.trim());
                  if (parts.length >= 4) { // Minimum degraded fields: front, ipa, wordForm, back
                    cardsInChunk.push({
                      front: parts[0] || "",
                      ipa: parts[1] || "",
                      wordForm: parts[2] || "",
                ...
                if (cardsInChunk.length < exactCount) {
                  ...
                  throw new Error(`Data Loss: Yêu cầu ${exactCount} thẻ nhưng chỉ nhận được ${cardsInChunk.length}. Đang làm dịu và thử lại...`);
                }

                success = true;
                const usedIndexStr = data.keyIndex ? ` qua Key #${data.keyIndex}` : "";
                pushLog(`✅ [Phân đoạn ${cIdx + 1}] Tạo thành công ${cardsInChunk.length}/${exactCount} thẻ${usedIndexStr}.`, true);
              } else {
                throw new Error(data.message || "Phản hồi lỗi do thất thoát chuỗi văn bản.");
              }
              ...
              if (err.name === 'AbortError') {
                if (isCanceledRef.current) {
                  pushLog(`🛑 Cuộc gọi phân đoạn ${cIdx + 1} đã bị dừng khẩn cấp.`, true);
                  break;
                } else {
                  pushLog(`🎯 Đã chủ động hủy và bỏ qua phân đoạn #${cIdx + 1}.`, true);
              ...
              pushLog(`⚠️ [Phân đoạn ${cIdx + 1}] Thử lần [${attempt}/${maxRetries}] thất bại: ${lastErrorMessage}`, true);

              if (attempt < maxRetries && !isCanceledRef.current) {
                const backoff = 4000 * Math.pow(2, attempt - 1);
                pushLog(`⏳ Hồi sức chờ ${backoff / 1000} giây trước khi tự truy vấn lại...`);
          ...
          if (success) {
            try {
              const batch = writeBatch(db);
              const targetDeckForBatch = isAddToExisting && selectedExistingDeckId ? store.getDeck(selectedExistingDeckId) : null;
              const currentBatchDeckId = targetDeckForBatch ? targetDeckForBatch.id : activeDeckId;
              const currentBatchSubject = targetDeckForBatch ? (targetDeckForBatch.subject || "Tự chọn") : (subject || "Tự chọn");
              ...
              pushLog(`🚨 Lỗi ghi Firestore tại phân đoạn ${cIdx + 1}: ${saveErr.message || saveErr}`, true);
            }
          } else {
            pushLog(`🚨 [Mảng Cách Ly] Tiếp tục cô lập phân đoạn lỗi ${cIdx + 1} do cạn hạn kết nối.`, true);
      ...
      let targetDeck = isAddToExisting && selectedExistingDeckId ? store.getDeck(selectedExistingDeckId) : null;
      ...
      if (cardsToUse.length > 0) {
        ...
        await store.addDeck(newDeckObj);
        if (targetDeck) {
          pushLog(`🎉 [Hoàn thành] Đã thêm thành công ${cardsToUse.length} thẻ mới vào bộ thẻ sẵn có "${newDeckObj.title}"!`, true);
        } else {
          pushLog(`🎉 [Hoàn thành] Đã tạo thành công bộ thẻ "${newDeckObj.title}" với ${cardsToUse.length} thẻ học!`, true);
        }
      ...
      if (failedChunksAccumulator.length > 0) {
        pushLog(`⚠️ Quá trình kết thúc với ${failedChunksAccumulator.length} phân đoạn bị cách ly lỗi.`, true);
      }
      ...
      setError(`Lỗi Pipeline: ${err.message || "Tự động lưu tiến trình cũ lỗi."}`);
    } finally {
      setIsAutomating(false);
      setAutoProgress(null);
    }
  ...
  return (
    <section 
      style={isEcoMode ? { transform: "translateZ(0)", willChange: "transform" } : undefined}
      className={cn(
        isEcoMode
    ...
    >
      ...
      {successCount !== null && (
        <div className={cn(
          ...
          <span>Bơm thành công {successCount} thẻ học vào Database! Trạng thái Realtime đã đồng bộ tệp.</span>
        </div>
      )}
      ...
      {activeTab === "automated" ? (
        <div 
          style={isEcoMode ? { transform: "translateZ(0)", willChange: "transform" } : undefined}
          className={cn("space-y-6", isEcoMode ? "transition-none animate-none duration-0" : "animate-in fade-in duration-350")}
        >
          <div className="bg-blue-500/5 border border-blue-500/10 rounded-xl p-4 flex gap-3 text-sm text-blue-700 dark:text-blue-400 leading-relaxed">
            ...
            <p>
              Dán đoạn văn bản chứa danh sách từ vựng thô, câu nói, tài liệu chuyên ngành... Hệ thống sẽ tự động băm nhỏ theo cơ chế <strong>Lazy Chunking</strong>, tự động trích xuất IPA, cụm ví dụ, cấu trúc dịch nghĩa, xoay tua khóa API lỗi và ghi trực tiếp vào Firestore.
            </p>
          </div>

          {/* Option: Thêm vào bộ thẻ đã có */}
          <div className="p-4 bg-blue-50/40 dark:bg-zinc-900/35 border border-blue-100/60 dark:border-zinc-800/60 rounded-xl space-y-3">
            <label className="flex items-center gap-2 cursor-pointer select-none">
              <input
                type="checkbox"
                checked={isAddToExisting}
                onChange={(e) => {
                  setIsAddToExisting(e.target.checked);
                  if (e.target.checked) {
                    const deks = store.getDecks();
                    if (deks.length > 0 && !selectedExistingDeckId) {
                      setSelectedExistingDeckId(deks[0].id);
                    }
                  }
                }}
                className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500"
              />
              <span className="text-sm font-bold text-stone-850 dark:text-stone-200">
                 Thêm thẻ mới vào một bộ thẻ đã có (Optional)
              </span>
            </label>
            ...
            {isAddToExisting && (
              <div className="space-y-2 pt-1 animate-in fade-in slide-in-from-top-1 duration-200">
                <span className="text-xs font-bold text-stone-500 block uppercase tracking-wider">
                   Chọn bộ thẻ học mục tiêu của bạn:
                </span>
                {store.getDecks().length === 0 ? (
                  <p className="text-xs text-stone-500 italic">Hiện tại chưa có bộ thẻ học nào trong thư viện.</p>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto pr-1.5 scrollbar-thin">
                    {store.getDecks().map((dk) => {
                      const isSelected = selectedExistingDeckId === dk.id;
                      return (
                        <button
                          key={dk.id}
                          type="button"
                          onClick={() => setSelectedExistingDeckId(dk.id)}
                          className={cn(
                            "p-3 rounded-lg border text-left flex flex-col justify-between transition cursor-pointer text-xs font-medium relative group",
                            isSelected
                              ? "bg-blue-500/10 border-blue-500 text-blue-700 dark:text-blue-300 font-bold"
                              : "bg-white dark:bg-black/50 border-stone-200 dark:border-zinc-800 hover:border-blue-400 dark:hover:border-blue-500/40 text-stone-700 dark:text-stone-300"
                          )}
                        >
                          <span className="block truncate font-bold text-stone-900 dark:text-white">
                            {dk.title}
                          </span>
                          <span className="text-[10px] opacity-60 block mt-1">
                            Danh mục: <span className="font-bold">{dk.subject || "Tự chọn"}</span> • Thẻ: <span className="font-bold">{dk.cards?.length || 0}</span>
                          </span>
                          {isSelected && (
                            <div className="absolute top-2 right-2 flex items-center justify-center bg-blue-500 text-white w-4 h-4 rounded-full">
          ...
          {!isAddToExisting && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in duration-200">
              <div>
                <label className="text-base font-semibold opacity-85 mb-2 block">Tên Bộ Thẻ Học Mới:</label>
                <input 
                  type="text" 
                  value={deckTitle}
                  onChange={(e) => setDeckTitle(e.target.value)}
                  placeholder="Ví dụ: 1000 Từ khóa Khoa Học Đời Sống"
                  className="w-full bg-white dark:bg-zinc-950 border border-stone-200/50 dark:border-zinc-800/80 rounded-xl px-4 py-3.5 text-stone-900 dark:text-outline text-stone-100 text-base focus:outline-none focus:ring-1 focus:ring-blue-500"
                />
              </div>
              <div>
                <label className="text-base font-semibold opacity-85 mb-2 block">Danh mục / Phân loại:</label>
                {!isCreatingNewSubject ? (
                  <div className="flex gap-2">
                    <select
                      ...
                      className="flex-1 bg-white dark:bg-zinc-950 border border-stone-200/50 dark:border-zinc-800/80 rounded-xl px-4 py-3.5 text-stone-900 dark:text-stone-100 text-base focus:outline-none focus:ring-1 focus:ring-blue-500"
                    >
                      ...
                      <option value="__NEW__" className="text-amber-500 font-bold">
                        + Thêm danh mục mới...
                      </option>
                    </select>
                    <button
                      ...
                      className="p-3 bg-stone-100 dark:bg-zinc-800 hover:bg-stone-200 dark:hover:bg-zinc-700 text-stone-900 dark:text-stone-100 rounded-xl flex items-center justify-center border border-stone-200/50 dark:border-zinc-800"
                    >
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="px-4 bg-stone-100 dark:bg-zinc-800 hover:bg-stone-200 dark:hover:bg-zinc-700 rounded-xl text-xs font-bold border border-stone-200/50 dark:border-zinc-800"
                    >
                      Quay lại
                    </button>
                  </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <h5 className={cn(
                    ...
                    isEcoMode ? "text-white" : "text-amber-700 dark:text-amber-400"
                  )}>
                    <AlertTriangle className={cn("w-5 h-5", isEcoMode ? "text-white" : "text-amber-500 animate-pulse")} /> Phát Hiện Tiến Trình Bị Gián Đoạn!
                  </h5>
                  <p className={cn(
                    "text-sm font-medium mt-1",
                    isEcoMode ? "text-white" : "text-slate-900 dark:text-stone-200"
                  )}>
                    Hệ thống đã tự động sao lưu tiến trình băm thẻ dở cho bộ <strong>"{savedPipeline.deckTitle}"</strong> (Phân đoạn: {savedPipeline.currentChunk}/{Math.ceil(savedPipeline.rawLines.length / 6)}). Bạn có muốn Auto-Resume không?
                  </p>
                </div>
                <div className="flex gap-2 shrink-0">
                  <button
                    ...
                    className={cn(
                      ...
                      isEcoMode ? "border border-amber-700 shadow-none transition-none animate-none duration-0" : "transition active:scale-95 animate-pulse"
                    )}
                  >
          ...
          <div>
            <label className="block text-base font-semibold opacity-85 mb-2">Nhập tệp dữ liệu từ vựng thô (Một từ/câu mỗi dòng là tốt nhất):</label>
            <textarea
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-zinc-800 pb-3 gap-2">
                <div className="flex flex-col">
                  <span className="flex items-center gap-1.5 text-blue-400 font-bold text-xs">
                    <Terminal className={cn("w-4 h-4 text-blue-500", !isEcoMode && "animate-pulse")} /> Trạng thái Hàng đợi AI
                  </span>
                  {autoProgress.totalLines && (
                    <span className="text-[11px] text-yellow-500 font-bold font-mono mt-0.5 animate-pulse">
                      Đang tạo thẻ: {autoProgress.processedCards} / {autoProgress.totalLines}
                    </span>
                  )}
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  {activeProcessingChunkIndex !== null && (
                    <button
                      type="button"
                      onClick={() => handleSkipActiveChunk(activeProcessingChunkIndex)}
                    ...
                    >
                      <X className="w-3 h-3" /> Hủy mảng {activeProcessingChunkIndex + 1}
                    </button>
                  )}
                </div>
              </div>
              ...
              <div className="pt-2 border-t border-zinc-800">
                <div className="flex items-center justify-between text-xs mb-1 mb-2 font-bold opacity-80">
                  ...
                  <span>{Math.round((autoProgress.currentChunk / autoProgress.totalChunks) * 100)}%</span>
                </div>
                <div className="w-full h-3 bg-zinc-800 rounded-full overflow-hidden">
                  <div 
                    className={cn(
                      "h-full rounded-full",
                      isEcoMode ? "bg-blue-600 transition-none duration-0" : "bg-gradient-to-r from-blue-500 to-indigo-500 transition-all duration-300"
                    )}
                    style={{ width: `${(autoProgress.currentChunk / autoProgress.totalChunks) * 100}%` }}
                  />
                </div>
                <p className="text-[11px] text-zinc-400 mt-2 text-right">
                  Đã xử lý: <strong className="text-emerald-400">{autoProgress.processedCards} / {autoProgress.totalLines}</strong> thẻ.
                </p>
              </div>
            </div>
          )}
            ...
            <div className="p-5 bg-white dark:bg-zinc-950 rounded-2xl border border-stone-200 dark:border-zinc-800 space-y-4">
              <div className="flex items-center justify-between border-b border-stone-100 dark:border-zinc-800 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-stone-800 dark:text-stone-200">Bảng Giám sát Telemetry & Phân tích Đột biến AI</h3>
                    <p className="text-xs text-stone-400 dark:text-zinc-500">Ghi nhận token, độ mất mát dữ liệu và luân phiên API keys theo thời gian thực</p>
                  </div>
                </div>
                <button
                  type="button"
                ...
                <table className="w-full text-left text-xs font-mono">
                  <thead>
                    <tr className="border-b border-stone-150 dark:border-zinc-800/80 text-stone-400 dark:text-zinc-500">
                      <th className="pb-2 font-semibold">Tập tin / Thời gian</th>
                      <th className="pb-2 font-semibold text-center">API Key Index</th>
                      <th className="pb-2 font-semibold text-center">Token Đã Dùng</th>
                      <th className="pb-2 font-semibold text-center">Số thẻ (Đạt/Yêu cầu)</th>
                      <th className="pb-2 font-semibold text-center">Trạng thái dữ liệu</th>
                      <th className="pb-2 font-semibold text-right">Phản hồi (Ms)</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-stone-100 dark:divide-zinc-900/50">
                    {telemetryLogs.map((log: any, idx: number) => {
                      ...
                      return (
                        <tr key={log.id || idx} className="hover:bg-stone-50 dark:hover:bg-zinc-900/20 transition-colors">
                          <td className="py-2.5 text-stone-500 dark:text-stone-400">
                            <div>{timeStr}</div>
                            <div className="text-[10px] text-stone-400 dark:text-zinc-650 font-mono">Len: {log.inputLength} kí tự</div>
                          </td>
                          <td className="py-2.5 text-center">
                            <span className="inline-flex items-center gap-1 bg-stone-100 dark:bg-zinc-900 px-2 py-0.5 rounded border border-stone-200 dark:border-zinc-850 font-bold text-[11px] text-blue-500">
                              🗝️ #{log.keyIndex} <span className="text-[10px] text-zinc-400">({log.keyMasked})</span>
                            </span>
                          </td>
                          <td className="py-2.5 text-center">
                            {log.status === "failed" ? (
                              <span className="text-red-500 font-extrabold text-[11px]">Bị huỷ / Lỗi</span>
                            ) : (
                              <div className="text-[11px]">
                                <span className="text-emerald-500 font-bold" title="Prompt Tokens">{log.tokenUsage?.promptTokens || 0}</span>
                                <span className="text-stone-400 mx-1">+</span>
                                <span className="text-indigo-500 font-bold" title="Completion Tokens">{log.tokenUsage?.completionTokens || 0}</span>
                                <span className="text-stone-400 mx-1">=</span>
                                <strong className="text-amber-500" title="Tổng Tokens">{log.tokenUsage?.totalTokens || 0}</strong>
                              </div>
                            )}
                          </td>
                          <td className="py-2.5 text-center font-bold">
                            {log.status === "failed" ? (
                              ...
                              <span className={cn(
                                log.isLossy ? "text-yellow-600 dark:text-yellow-400" : "text-emerald-500"
                              )}>
                                {log.actualCardsCount}
                              </span>
                            )}
                            <span className="text-stone-400 dark:text-zinc-600 font-normal"> / {log.targetMin}-{log.targetMax}</span>
                          </td>
                          <td className="py-2.5 text-center">
                            {log.status === "failed" ? (
                              <span className="inline-flex items-center gap-1 text-red-500 bg-red-50 dark:bg-red-950/20 px-2 py-0.5 rounded border border-red-200 dark:border-red-900/30 text-[10px] font-bold" title={log.errorMessage}>
                                ⚠️ Thất bại
                              </span>
                            ) : log.isLossy ? (
                              <span className="inline-flex items-center gap-1 text-yellow-600 bg-yellow-50 dark:bg-yellow-950/20 px-2 py-0.5 rounded border border-yellow-200 dark:border-yellow-905/30 text-[10px] font-bold" title="Số thẻ tạo ra thấp hơn mức tối thiểu yêu cầu cho cụm này.">
                                🟡 Mất thông tin (Lossy)
                              </span>
                            ) : (
                          ...
                          <td className="py-2.5 text-right font-bold text-stone-600 dark:text-neutral-400">
                            {log.latencyMs ? `${(log.latencyMs / 1000).toFixed(1)}s` : "Chờ"}
                          </td>
                        </tr>
                      );
                    })}
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              ...
              disabled={isAutomating || !rawTextLines.trim() || (!isAddToExisting && !deckTitle.trim()) || (isAddToExisting && !selectedExistingDeckId)}
              className={cn(
                ...
                isEcoMode ? "shadow-none transition-none duration-0" : "transition shadow-lg shadow-blue-500/10"
              )}
            >
              {isAutomating ? (
                <>
                  <RefreshCw className={cn("w-5 h-5", !isEcoMode && "animate-spin")} /> Đang Chạy Pipeline ({autoProgress ? `${autoProgress.currentChunk}/${autoProgress.totalChunks}` : "Song song"})...
                </>
              ) : (
                <>
                  <Sparkles className="w-5 h-5 text-yellow-300" /> Kích Hoạt Kênh Tự Động Hoá (Hỗ trợ Song song CC)
                </>
              )}
            </button>
            {isAutomating && (
              <button
                ...
                className={cn(
                  ...
                  isEcoMode ? "bg-red-700 text-white shadow-none" : "hover:shadow-lg animate-pulse"
                )}
              >
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-stone-250 dark:border-zinc-800 pb-2">
                <span className={cn(
                  ...
                  isEcoMode ? "text-emerald-600 dark:text-emerald-450" : "text-emerald-600 dark:text-emerald-450"
                )}>
                  <Check className={cn("w-5 h-5", !isEcoMode && "animate-bounce")} /> Các Thẻ Đã Tạo Thành Công ({autoCreatedCards.length} Thẻ)
                </span>
              ...
              {autoCreatedCards.length > 30 && (
                <p className="text-[11px] text-amber-500 font-bold mb-2">
                  ⚠️ Đang hiển thị 30 thẻ mới nhất được băm để xem và chỉnh sửa nhanh. Toàn bộ {autoCreatedCards.length} thẻ vẫn được tự động ghi nhận trực tiếp realtime vào cơ sở dữ liệu.
                </p>
              )}
              ...
              >
                {autoCreatedCards.slice(-30).map((c, index) => (
                  <div key={c.id || index} className={cn(
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pl-1 pr-6 text-xs">
                      <div>
                        <span className="text-[9px] uppercase font-bold tracking-wider opacity-60 mb-0.5 block">Mặt Trước (Front)</span>
                        <input
                          type="text"
                          value={c.front}
                          onChange={(e) => handleUpdateAutoCard(c.id, 'front', e.target.value)}
                      ...
                      <div>
                        <span className="text-[9px] uppercase font-bold tracking-wider opacity-60 mb-0.5 block">Mặt Sau (Back)</span>
                        <textarea
                          value={c.back}
                          rows={1}
                          onChange={(e) => handleUpdateAutoCard(c.id, 'back', e.target.value)}
            ...
            <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-xl space-y-3 shadow-sm">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-red-500/20 pb-2">
                <span className="flex items-center gap-2 font-bold text-red-500 text-sm">
                  <AlertCircle className="w-5 h-5" /> Báo Cáo Cách Ly Sai Lệch ({pipelineErrors.length} Phân Đoạn Lỗi)
                </span>
                <div className="flex flex-wrap gap-2">
                  ...
                  >
                    {retryAllLoading ? (
                      <>
                        <RefreshCw className="w-3.5 h-3.5 animate-spin" /> Đang băm lại hàng loạt...
                      </>
                    ) : (
                      <>
                        <Sparkles className="w-3.5 h-3.5" /> Băm lại tất cả phân đoạn lỗi
                      </>
                    )}
                  </button>
                  <button
                    onClick={() => {
                      const blob = new Blob([JSON.stringify(pipelineErrors, null, 2)], { type: "application/json" });
              ...
              <div className="max-h-72 overflow-y-auto space-y-3 pr-1 scrollbar-thin">
                {pipelineErrors.map((errItem, idx) => {
                  ...
                  const isRetryingCurrent = processingErrorIndex === errItem.chunkIndex;

                  return (
                    <div key={idx} className={cn(
                      "p-3 bg-red-550/5 dark:bg-black/40 rounded-lg border border-red-500/10 space-y-2 relative group",
                      !isEcoMode && "hover:bg-red-500/10 transition duration-150"
                    )}>
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 text-xs text-red-400 font-bold">
                        ...
                        <span className="text-[11px] opacity-80 font-normal">{errItem.error}</span>
                      </div>
                      ...
                      {isEditing ? (
                        <div className="space-y-2">
                          <textarea
                            ...
                            onChange={(e) => setEditingErrorText(e.target.value)}
                          />
                          <div className="flex justify-end gap-2">
                            ...
                            <button
                              ...
                              onClick={() => handleRetrySingleFailedChunk(idx, errItem.chunkIndex, editingErrorText)}
                        ...
                        <>
                          ...
                          <div className="flex justify-end gap-2 text-xs pt-1">
                            ...
                            <button
                              disabled={isRetryingCurrent || processingErrorIndex !== null}
                              onClick={() => handleRetrySingleFailedChunk(idx, errItem.chunkIndex, errItem.content)}
                            ...
                            >
                              {isRetryingCurrent ? (
                                ...
                                <>
                                  <RefreshCw className="w-3 h-3" /> Chạy lại ngay
                                </>
                              )}
                            </button>
                          </div>
        ...
        <div 
          style={isEcoMode ? { transform: "translateZ(0)", willChange: "transform" } : undefined}
          className={cn("space-y-6", isEcoMode ? "transition-none animate-none duration-0" : "animate-in fade-in duration-300")}
        >
          {!previewCards ? (
            <div className="space-y-4">
              <label className="block text-base font-semibold opacity-85 mb-2">Tải tệp JSON/TXT hoặc dán chuỗi JSON vào đây:</label>
              
              <p className="text-sm text-red-500 dark:text-red-400 font-medium mb-2">
                ⚠️ Phương án dự phòng có thể xảy ra lỗi nếu AI bị đứt gãy giữa chừng trong lúc sinh JSON siêu dài.
              </p>
              ...
              <div className="flex flex-col items-start gap-3 mb-4">
                ...
                >
                  <Sparkles className="w-3.5 h-3.5" /> Chuẩn hoá chuỗi JSON dự phòng
                </button>
                  ...
                  <div className={cn(
                    ...
                    <button 
                      ...
                      className={cn(
                        ...
                        isEcoMode ? "transition-none shadow-none font-extrabold duration-0" : "transition"
                      )}
                      id="gemini-copy-btn"
                    >
                      {copiedNormalize ? <Check className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
                      {copiedNormalize ? "Đã Copy & Chuyển..." : "Copy Prompt Dự Phòng & Chuyển Sang Gemini"}
              ...
              <div 
                ...
                className={cn(
                  ...
                  isEcoMode
                    ...
                    : "border-stone-300 dark:border-zinc-800 bg-stone-50/50 dark:bg-zinc-950/20 hover:bg-stone-50 dark:hover:bg-zinc-950/40 hover:border-blue-500 dark:hover:border-blue-500/50 transition duration-200 animate-in fade-in duration-300"
                )}
                id="manual-drop-zone"
              >
                <input 
                ...
                {jsonFileName ? (
                  <>
                    ...
                    <p className="text-base font-bold text-stone-800 dark:text-stone-200">
                      Đã nạp thành công: <span className="text-emerald-500">{jsonFileName}</span>
                    </p>
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
                  ...
                  <>
                    <FileUp className={cn("w-10 h-15 text-blue-500/80", !isEcoMode && "animate-pulse")} />
                    <p className="text-base font-bold text-stone-700 dark:text-stone-300">
                      Kéo thả hoặc nhấn để tải tệp JSON / TXT lên
                    </p>
                    <p className="text-xs opacity-60">
                      Nhấn vào đây để tải tệp .txt hoặc .json chứa dữ liệu thẻ học của bạn
                    </p>
                  </>
                )}
              </div>
              ...
              <div className="space-y-1">
                <label className="block text-sm font-semibold opacity-85">Hoặc dán tay chuỗi JSON vào đây:</label>
                <textarea
              ...
              <div className="flex gap-4 items-center justify-center pt-2 w-full">
                ...
                >
                  {isAiParsingJson ? (
                    <>
                      <Loader2 className={cn("w-5 h-5", !isEcoMode && "animate-spin")} /> AI Đang Sửa Lỗi Cú Pháp...
                    </>
                  ) : (
                    <>
                      <Sparkles className="w-4 h-4 text-white" /> Sửa lỗi & Chuẩn hóa cấu trúc bằng AI (Khuyên dùng)
                    </>
                  )}
                </button>
              </div>
            ...
            <div 
              ...
              className="space-y-6"
            >
              <div className={cn("flex items-center justify-between mt-2", isEcoMode ? "transition-none animate-none duration-0" : "animate-in slide-in-from-top-2 duration-300")}>
                ...
                <button 
                  ...
                  className={cn(
                    ...
                    isEcoMode ? "transition-none shadow-none text-red-600/90 font-bold duration-0" : "transition"
                  )}
                >
                  Hủy / Sửa JSON
                </button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in duration-200">
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-white dark:bg-zinc-950 border border-stone-200/50 dark:border-zinc-800/80 rounded-xl px-4 py-3 text-stone-900 dark:text-stone-100 text-sm focus:outline-none"
                    />
                  </div>
                  <div>
                    ...
                    {!isCreatingNewSubject ? (
                      <div className="flex gap-2">
                        <select
                          ...
                          className="flex-1 bg-white dark:bg-zinc-950 border border-stone-200/50 dark:border-zinc-800/80 rounded-xl px-4 py-3 text-stone-900 dark:text-stone-100 text-sm"
                        >
                          <option value="">-- Chọn danh mục hiện có --</option>
                          {existingCategories.map((cat, idx) => (
                            <option key={idx} value={cat}>
                          ...
                          <option value="__NEW__" className="text-amber-600 dark:text-yellow-450 font-bold">
                            + Thêm danh mục mới...
                          </option>
                        </select>
                      </div>
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 bg-white dark:bg-zinc-950 border border-stone-200/50 dark:border-zinc-800/80 rounded-xl px-4 py-3 text-stone-900"
                        />
                      </div>
                    )}
                  </div>
              ...
              {previewCards.length > 40 && (
                <p className="text-[11px] text-amber-500 font-bold mb-2">
                  ⚠️ Đang hiển thị bản nháp xem trước của 40 thẻ đầu tiên để tối ưu hóa hiệu năng thiết bị. Toàn bộ {previewCards.length} thẻ vẫn được nạp đầy đủ vào hệ thống.
                </p>
              )}
                  ...
                  <div key={card.id} className={cn(
                    ...
                    <button 
                      ...
                      className={cn(
                        ...
                        isEcoMode ? "transition-none shadow-none duration-0" : "transition"
                      )}
                    >
                    ...
                    <div className="pl-3 pr-6 space-y-3">
                      <div>
                        <label className={cn("text-[10px] uppercase font-bold tracking-wider opacity-50 mb-1 block", !isEcoMode && "animate-in fade-in")}>Front</label>
                        <input
                          type="text"
                          value={card.front}
              ...
              <div className="pt-4 border-t border-stone-200/50 dark:border-zinc-800">
                ...
                <button 
                  ...
                  disabled={isProcessing || previewCards.length === 0 || (!isAddToExisting && !deckTitle.trim()) || (isAddToExisting && !selectedExistingDeckId)}
                  className={cn(
                    ...
                    isEcoMode ? "transition-none shadow-none animate-none duration-0" : "transition animate-pulse"
                  )}
                >
                  {isProcessing ? "Đang Đồng Bộ..." : `Kích Hoạt Nạp ${previewCards.length} Thẻ Học`}
                </button>
              </div>
            </div>
          )}
          ...
          <div className={cn(
            ...
            <div className="px-6 py-4 flex justify-between items-center border-b border-stone-200/50 dark:border-zinc-800/50">
              <h3 className="font-display font-semibold text-lg flex items-center gap-2 text-stone-900 dark:text-stone-100">
                <Sparkles className="w-5 h-5 text-indigo-500" /> Bản Thiết Kế Prompt Hệ Cực Hạn
              </h3>
              <button 
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className={cn(
                ...
                <p className="text-sm opacity-100 font-medium mb-4 flex-grow text-stone-600 dark:text-stone-300">
                  Google AI Studio chuyên dụng đã tối giản hoá token và tự động ghi chú mảng.
                </p>
                <a 
                  ...
                  className={cn(
                    ...
                    isEcoMode ? "transition-none shadow-none font-bold duration-0" : "transition"
                  )}
                >
                  Mở Google AI Studio
                </a>
              ...
              <div className={cn(
                ...
                <p className="text-sm opacity-100 font-medium mb-4 flex-grow text-stone-600 dark:text-stone-300">
                  Sử dụng Prompt tối ưu hóa để gò ép cấu trúc mảng JSON hoàn hảo khi làm thủ công.
                </p>
                <div className="space-y-2 mt-auto">
                  ...
                  >
                    ...
                    {copied ? "Đã Copy Prompt..." : "Sao Chép Prompt & Mở Gemini Web"}
                  </button>
                </div>
              </div>
            </div>
