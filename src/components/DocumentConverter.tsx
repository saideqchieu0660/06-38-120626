...
const loadPdfJS = () => {
  return new Promise<any>((resolve, reject) => {
    ...
    script.onerror = () => {
      reject(new Error("Không thể tải thư viện xử lý PDF.js từ CDN. Vui lòng kiểm tra kết nối mạng của mày."));
    };
...
export default function DocumentConverter() {
  ...
  const [activeImportTab, setActiveImportTab] = useState<"file" | "text" | "json">("file");
  ...
  const pushLog = (msg: string, immediate = false) => {
    ...
    const now = Date.now();
    if (immediate || now - lastLogUpdateRef.current >= 600) {
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      ...
      if (selected.size > 10 * 1024 * 1024) {
        setError("Kích thước tệp vượt quá 10MB. Vùi lòng sử dụng file nhẹ hơn.");
        return;
      }
      setFile(selected);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      if (selected.size > 10 * 1024 * 1024) {
        setError("Kích thước tệp vượt quá 10MB. Vui lòng sử dụng file nhẹ hơn.");
        return;
      }
      setFile(selected);
      setUploadedFileName(selected.name.replace(/\.[^/.]+$/, ""));
    ...
    try {
      ...
      if (activeImportTab === "file") {
        ...
        if (file.type === "text/plain" || file.name.endsWith(".txt")) {
          ...
          pushLog("🛰️ Đang vận chuyển sang Gateway trích xuất AI Multimodal (Extracting API)...");
          setProgressText("Đang nhận dạng văn bản...");
          ...
          extractedSourceText = extractResData.rawText || "";
          fileNameTarget = file.name;
        }
      } else {
        // Tab Text Ingestion
  ...
  return (
    <section className="card-3d rounded-3xl p-6 md:p-8 relative bg-white dark:bg-zinc-950 border border-stone-200/50 dark:border-zinc-800/50 shadow-xl flex flex-col gap-6 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-3 p-1.5 bg-stone-50 dark:bg-zinc-900/60 rounded-2xl border border-stone-200/60 dark:border-zinc-800/50">
        <button
        ...
        >
          <span className="text-sm">⚙️</span> DÃN CHUỖI JSON
        </button>
      </div>
             ...
             >
                ...
                {file ? (
                   ...
                   <>
                      ...
                      <p className="text-xs opacity-60 mt-1.5 leading-relaxed">Nhấn hoặc Kéo thả file PDF, Ảnh hoặc File TEXT (.txt) vào đây.<br />Tối đa 10MB cho mỗi lượt.</p>
                   </>
                )}
             </div>
          )}
...
[
  { "front": "hello", "back": "xin chào" }
             ...
             <div className="p-4 bg-stone-50 dark:bg-zinc-900/35 border border-stone-200/60 dark:border-zinc-800/70 rounded-2xl space-y-3.5">
                ...
                <div className="pb-3 border-b border-stone-250/65 dark:border-zinc-800 flex flex-col gap-1.5 animate-in fade-in duration-300">
                  ...
                  >
                     ...
                     <option value={3}>🚀 Hỏa tốc (3 luồng - Tải nhanh mượt mà)</option>
                     <option value={4}>🚀🚀 Phi thuyền (4 luồng - Siêu tốc độ, dành cho file cực lớn)</option>
                     <option value={6}>🛸 Tốc ánh sáng (6 luồng - Bứt phá 1000 thẻ chỉ dưới 2 phút! Kỷ lục)</option>
                  </select>
                </div>
        ...
        <div className="space-y-4">
           {/* Add to exists set configurations */}
              ...
              {isAddToExisting && (
                <div className="space-y-2 pt-1 animate-in fade-in slide-in-from-top-1 duration-150">
                  {store.getDecks().length === 0 ? (
                    <p className="text-xs text-stone-400 italic">Hiện tại chưa có bộ thẻ học nào trong thư viện.</p>
                  ) : (
                      ...
                      {store.getDecks().map((dk) => {
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              "p-3 rounded-xl border text-left flex justify-between items-center transition cursor-pointer text-xs group relative",
                              isSelected
                                ? "bg-yellow-500/10 border-yellow-500 text-yellow-700 dark:text-yellow-400 font-bold"
                                : "bg-white dark:bg-black/40 border-stone-200 dark:border-zinc-800 hover:border-yellow-450/40 text-stone-700 dark:text-stone-300"
                            )}
                          >
                            <div className="truncate pr-4">
                              <span className="block font-bold text-stone-900 dark:text-white truncate">
                                {dk.title}
                              </span>
                              <span className="text-[10px] opacity-60 block mt-0.5">
                                Danh mục: <strong>{dk.subject || "Tự chọn"}</strong> • Thẻ: <strong>{dk.cards?.length || 0}</strong>
                              </span>
                            </div>
                            {isSelected && (
                              <div className="flex items-center justify-center bg-yellow-500 text-stone-950 w-4.5 h-4.5 rounded-full shadow-sm shrink-0">
